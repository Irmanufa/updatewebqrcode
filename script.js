...
// VERSI TERBARU DENGAN SEMUA PERBAIKAN
// ============================================
...
function showAlreadyAttendedDialog(member) {
  ...
  scanResultDiv.innerHTML = `<div style="text-align: center;"><i class="fas fa-exclamation-triangle" style="font-size: 48px; color: #f59e0b; margin-bottom: 10px;"></i><div class="member-name-large" style="color: #92400e;">${member.name}</div><div class="member-info-grid"><span class="info-badge"><i class="fas fa-id-card"></i> ${member.code}</span><span class="info-badge"><i class="fas fa-building"></i> ${member.division}</span><span class="info-badge"><i class="fas fa-clock"></i> ${todayAttendance?.time || ""}</span></div><div style="margin-top: 10px; font-size: 16px; font-weight: bold;">⚠️ SUDAH ABSEN HARI INI!</div></div><div class="scan-actions"><button class="btn btn-warning btn-sm" onclick="editAttendanceById(${todayAttendance?.id})"><i class="fas fa-edit"></i> Edit Data Absen</button><button class="btn btn-danger btn-sm" onclick="deleteAttendanceById(${todayAttendance?.id})"><i class="fas fa-trash"></i> Hapus & Absen Ulang</button></div>`;
}

function showScanResult(member, attendanceId, isSuccess = true) {
  const scanResultDiv = document.getElementById("scanResult");
  if (!scanResultDiv) return;
  if (isSuccess) {
    scanResultDiv.className = "scan-result success";
    scanResultDiv.innerHTML = `<div style="text-align: center;"><i class="fas fa-check-circle" style="font-size: 48px; color: #059669; margin-bottom: 10px;"></i><div class="member-name-large">${member.name}</div><div class="member-info-grid"><span class="info-badge"><i class="fas fa-id-card"></i> ${member.code}</span><span class="info-badge"><i class="fas fa-building"></i> ${member.division}</span><span class="info-badge"><i class="fas fa-user-tag"></i> ${member.position || "Anggota"}</span><span class="info-badge"><i class="fas fa-clock"></i> ${formatTime()}</span></div><div style="margin-top: 10px; font-size: 14px;"><i class="fas fa-check-circle" style="color: #059669;"></i> Absen Berhasil!</div></div><div class="scan-actions"><button class="btn btn-warning btn-sm" onclick="editAttendanceById(${attendanceId})"><i class="fas fa-edit"></i> Edit Data</button><button class="btn btn-danger btn-sm" onclick="deleteAttendanceById(${attendanceId})"><i class="fas fa-trash"></i> Hapus Absen</button></div>`;
  } else {
    scanResultDiv.className = "scan-result error";
    scanResultDiv.innerHTML = `<div style="text-align: center;"><i class="fas fa-times-circle" style="font-size: 48px; color: #ef4444; margin-bottom: 10px;"></i><div class="member-name-large" style="color: #991b1b;">${member.name}</div><div class="member-info-grid"><span class="info-badge"><i class="fas fa-id-card"></i> ${member.code}</span><span class="info-badge"><i class="fas fa-building"></i> ${member.division}</span></div><div style="margin-top: 10px; font-size: 16px; font-weight: bold;">❌ ${member.status === "passive" ? "ANGGOTA PASIF - TIDAK BISA ABSEN!" : "QR Code Tidak Valid!"}</div></div>`;
  }
  AppState.lastScanData = { member, attendanceId };
}
...
function showEditAttendanceModal(attendance) {
  ...
  modal.innerHTML = `<div class="modal"><div class="modal-header"><h3><i class="fas fa-edit"></i> Edit Data Absensi</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()">&times;</button></div><div class="input-group"><label>Nama Anggota</label><input type="text" value="${attendance.memberName}" disabled style="background:#f1f5f9;"></div><div class="input-group"><label>Divisi</label><input type="text" value="${attendance.division}" disabled style="background:#f1f5f9;"></div><div class="input-group"><label>Waktu Absensi</label><input type="time" id="editTime" value="${attendance.time}"></div><div class="input-group"><label>Tanggal Absensi</label><input type="date" id="editDate" value="${attendance.dateShort.split("/").reverse().join("-")}"></div><div class="input-group"><label>Nama Kegiatan</label><input type="text" id="editKegiatan" value="${attendance.kegiatan || AppState.currentKegiatan}"></div><div class="modal-buttons"><button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()">Batal</button><button class="btn btn-primary" onclick="saveEditAttendance(${attendance.id})">Simpan Perubahan</button></div></div>`;
...
function saveEditAttendance(attendanceId) {
  ...
  const updatedAttendance = AppState.attendance.find(
    (a) => a.id == attendanceId,
  );
  if (updatedAttendance) {
    const member = getMemberById(updatedAttendance.memberId);
    showScanResult(member, attendanceId, true);
  }
}
...
async function startScanner() {
  ...
  html5QrCode
    .start({ facingMode: "environment" }, config, (decodedText) => {
      ...
      if (member) {
        if (member.status === "active") {
          ...
          showScanResult(member, null, false);
          playBeepError();
        }
      } else {
        const scanResultDiv = document.getElementById("scanResult");
        if (scanResultDiv) {
          scanResultDiv.className = "scan-result error";
          scanResultDiv.innerHTML = `<div style="text-align: center;"><i class="fas fa-times-circle" style="font-size: 48px; color: #ef4444; margin-bottom: 10px;"></i><div style="font-size: 20px; font-weight: bold;">QR Code Tidak Valid!</div><div style="margin-top: 10px;">Pastikan QR Code adalah milik anggota IRMANUFA</div></div>`;
          playBeepError();
        }
      }
    })
    .catch((err) => {
      container.innerHTML =
        '<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><p>Gagal mengakses kamera! Pastikan izin kamera diberikan.</p></div>';
...
function generateMemberQR() {
  ...
  QRCode.toCanvas(
    ...
    function (error, canvas) {
      ...
      if (infoContainer) {
        infoContainer.innerHTML = `<div class="member-info-card"><h4><i class="fas fa-id-card"></i> DATA DIRI ANGGOTA</h4><p><strong>Nama:</strong> ${member.name}</p><p><strong>Jenis Kelamin:</strong> ${getGenderText(member.gender)}</p><p><strong>Kode Member:</strong> ${member.code}</p><p><strong>Jabatan:</strong> ${member.position || "Anggota"}</p><p><strong>Divisi:</strong> ${member.division}</p><p><strong>Status:</strong> <span class="status-badge status-active">AKTIF</span></p></div>`;
      }
...
function showDeleteConfirmModal(id) {
  ...
  const modal = document.createElement("div");
  modal.className = "modal-overlay";
  modal.innerHTML = `<div class="modal" style="max-width: 400px;"><div class="modal-header" style="background: #fee2e2; border-bottom-color: #ef4444;"><h3 style="color: #dc2626;"><i class="fas fa-trash-alt"></i> Hapus Anggota</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()">&times;</button></div><div style="text-align: center; padding: 20px 0;"><i class="fas fa-exclamation-triangle" style="font-size: 48px; color: #f59e0b; margin-bottom: 16px;"></i><p style="font-size: 16px; font-weight: 600; color: #1e293b;">Apakah Anda yakin?</p><p style="color: #64748b; margin-top: 8px;">Menghapus anggota <strong>${member.name}</strong><br>Data ini tidak dapat dikembalikan!</p></div><div class="modal-buttons" style="justify-content: center; gap: 16px;"><button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()" style="padding: 10px 24px;">Batal</button><button class="btn btn-danger" onclick="confirmDeleteMember('${id}')" style="padding: 10px 24px;">Hapus</button></div></div>`;
  document.body.appendChild(modal);
}
...
function showAddMemberModal() {
  const modal = document.createElement("div");
  modal.className = "modal-overlay";
  modal.innerHTML = `<div class="modal"><div class="modal-header"><h3><i class="fas fa-user-plus"></i> Tambah Anggota Baru</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()">&times;</button></div><div class="input-group"><label>Kode Member</label><input type="text" id="memberCode" placeholder="Contoh: 26.09.001"></div><div class="input-group"><label>Nama Lengkap</label><input type="text" id="memberName" placeholder="Nama lengkap"></div><div class="input-group"><label>Jenis Kelamin</label><select id="memberGender"><option value="L">Laki-laki</option><option value="P">Perempuan</option></select></div><div class="input-group"><label>Jabatan</label><input type="text" id="memberPosition" placeholder="Jabatan"></div><div class="input-group"><label>Divisi</label><input type="text" id="memberDivision" placeholder="Nama Divisi"></div><div class="input-group"><label>Status</label><select id="memberStatus"><option value="active">Aktif</option><option value="passive">Pasif</option></select></div><div class="modal-buttons"><button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()">Batal</button><button class="btn btn-primary" onclick="saveNewMember()">Simpan</button></div></div>`;
  document.body.appendChild(modal);
}
...
function renderMemberTable(searchTerm = "") {
  ...
  container.innerHTML = members
    .map(
      (m) =>
        `<tr><td>${m.code}</td><td><strong>${m.name}</strong><br><small>${getGenderText(m.gender)}</small></td><td>${m.division}</td><td>${m.position || "Anggota"}</td><td><span class="status-badge ${m.status === "active" ? "status-active" : "status-passive"}">${m.status === "active" ? "Aktif" : "Pasif"}</span></td><td><span class="status-badge ${todayAttended.includes(m.id) ? "status-hadir" : "status-belum"}">${todayAttended.includes(m.id) ? "Hadir" : "Belum"}</span></td><td><button class="btn btn-warning" style="padding: 4px 10px; margin-right: 5px;" onclick="showEditMemberModal('${m.id}')"><i class="fas fa-edit"></i></button><button class="btn btn-danger" style="padding: 4px 10px;" onclick="showDeleteConfirmModal('${m.id}')"><i class="fas fa-trash"></i></button></td></tr>`,
    )
    .join("");
}
...
function renderTopMembers() {
  ...
  container.innerHTML = stats.topMembers
    .map(
      (m, i) =>
        `<div class="ranking-item"><div class="rank-number" style="background: ${i === 0 ? "#f59e0b" : i === 1 ? "#94a3b8" : "#059669"};">${i + 1}</div><div class="rank-info"><h4>${m.name}</h4><div class="rank-stats"><span><i class="fas fa-check-circle"></i> ${m.count} kali</span><span><i class="fas fa-users"></i> ${m.division}</span></div></div></div>`,
    )
    .join("");
}
...
function updateCharts() {
  ...
  if (ctx2 && Object.keys(stats.divisionStats).length > 0) {
    ...
    const attendanceRates = divisionNames.map(
      (d) =>
        (stats.divisionStats[d].attended / stats.divisionStats[d].total) * 100,
    );
    const divisionColors = divisionNames.map(
...
function resetMembersToDefault() {
  showConfirmModal(
    "Reset Data Anggota",
    "Apakah Anda yakin ingin mereset SEMUA data anggota ke default?",
    () => {
...
function showConfirmModal(title, message, onConfirm) {
  const modal = document.createElement("div");
  modal.className = "modal-overlay";
  modal.innerHTML = `<div class="modal" style="max-width: 400px;"><div class="modal-header" style="background: #fef3c7; border-bottom-color: #f59e0b;"><h3 style="color: #d97706;"><i class="fas fa-exclamation-triangle"></i> ${title}</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()">&times;</button></div><div style="text-align: center; padding: 20px 0;"><i class="fas fa-question-circle" style="font-size: 48px; color: #f59e0b; margin-bottom: 16px;"></i><p style="color: #64748b;">${message}</p></div><div class="modal-buttons" style="justify-content: center; gap: 16px;"><button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()" style="padding: 10px 24px;">Batal</button><button class="btn btn-danger" id="confirmDeleteBtn" style="padding: 10px 24px;">Ya, Hapus</button></div></div>`;
  document.body.appendChild(modal);
...
function showSection(section) {
  ...
  if (section === "reports") {
    ...
    if (kegiatanInput) {
      ...
      kegiatanInput.addEventListener("change", (e) =>
...
function renderDashboard() {
  ...
  document.getElementById("app").innerHTML =
    `<div class="dashboard"><nav class="navbar"><div class="nav-content"><div class="nav-brand"><div class="logo-icon"><i class="fas fa-mosque"></i></div><div><h1>IRMANUFA QR Absensi</h1><small>Kabinet Golden Generation</small></div></div><div class="nav-stats"><span class="stat-badge"><i class="fas fa-users"></i> ${activeCount} Anggota Aktif</span><span class="stat-badge"><i class="fas fa-calendar-check"></i> ${stats.todayCount} Hadir Hari Ini</span><span class="stat-badge"><i class="fas fa-user-clock"></i> ${passiveCount} Pasif</span></div><button class="hamburger-btn" onclick="toggleMenu()"><i class="fas fa-bars"></i></button></div></nav><div class="sidebar"><div class="sidebar-header"><div class="user-avatar"><i class="fas fa-user-tie"></i></div><div class="user-info"><h3>Admin IRMANUFA</h3><p>Administrator Sistem</p><div class="user-badge"><i class="fas fa-shield-alt"></i> Super Admin</div></div></div><div class="sidebar-menu"><a href="#" class="menu-item active" data-section="dashboard" onclick="showSection('dashboard')"><i class="fas fa-home"></i><span>Dashboard</span></a><a href="#" class="menu-item" data-section="scanner" onclick="showSection('scanner')"><i class="fas fa-qrcode"></i><span>Scan QR Code</span><span class="menu-badge">New</span></a><a href="#" class="menu-item" data-section="generator" onclick="showSection('generator')"><i class="fas fa-print"></i><span>Cetak QR Code</span></a><a href="#" class="menu-item" data-section="members" onclick="showSection('members')"><i class="fas fa-users"></i><span>Data Anggota</span></a><a href="#" class="menu-item" data-section="reports" onclick="showSection('reports')"><i class="fas fa-chart-line"></i><span>Laporan</span></a><a href="#" class="menu-item" onclick="logout()"><i class="fas fa-sign-out-alt"></i><span>Keluar</span></a></div></div><div class="sidebar-overlay" onclick="closeMenu()"></div><div class="main-content"><div class="welcome-card"><div class="welcome-text"><h2>Selamat Datang, Admin!</h2><p>Sistem Absensi Digital Berbasis QR Code IRMANUFA</p><small><i class="fas fa-qrcode"></i> Scan QR Code untuk absensi cepat | <i class="fas fa-check-circle"></i> Hanya anggota AKTIF yang bisa absen</small></div><div class="date-info"><div class="time" id="currentTime">${formatTime()}</div><div class="date" id="currentDate">${formatDate()}</div></div></div><section id="dashboardSection" class="content-section active"><div class="stats-grid"><div class="stat-card"><div class="stat-icon"><i class="fas fa-users"></i></div><div class="stat-number" id="activeMembers">${stats.activeMembers}</div><div class="stat-label">Anggota Aktif</div></div><div class="stat-card"><div class="stat-icon"><i class="fas fa-calendar-day"></i></div><div class="stat-number" id="todayCount">${stats.todayCount}</div><div class="stat-label">Hadir Hari Ini</div></div><div class="stat-card"><div class="stat-icon"><i class="fas fa-chart-line"></i></div><div class="stat-number" id="totalAttendance">${stats.totalAttendance}</div><div class="stat-label">Total Absensi</div></div></div><div class="card"><div class="card-header"><i class="fas fa-chart-line"></i><h3>Tren Absensi 7 Hari Terakhir</h3></div><div class="chart-wrapper"><canvas id="attendanceChart"></canvas></div></div><div class="card"><div class="card-header"><i class="fas fa-chart-pie"></i><h3>Persentase Kehadiran per Divisi</h3><small style="color:#64748b;">(Warna berbeda setiap divisi)</small></div><div class="chart-wrapper"><canvas id="divisionChart"></canvas></div></div><div class="card"><div class="card-header"><i class="fas fa-trophy"></i><h3>Top 5 Anggota Paling Rajin</h3></div><div id="topMembersList" class="ranking-list"></div></div><div class="card"><div class="card-header"><i class="fas fa-history"></i><h3>Absensi Terbaru</h3></div><div id="recentAttendanceList" class="ranking-list"></div></div></section><section id="scannerSection" class="content-section"><div class="card"><div class="card-header"><i class="fas fa-qrcode"></i><h3>Scan QR Code Absensi</h3><small>Arahkan kamera ke QR Code anggota</small></div><div class="scanner-header"><button id="flashToggleBtn" class="flash-btn" onclick="toggleFlash()"><i class="fas fa-lightbulb"></i> Flash OFF</button></div><div class="scanner-container"><div id="reader" class="empty-state"><i class="fas fa-camera"></i><p>Scanner siap. Klik "Mulai Scan"</p></div></div><div id="scanResult" class="scan-result"></div><div class="action-buttons"><button class="btn btn-primary" onclick="startScanner()"><i class="fas fa-play"></i> Mulai Scan</button><button class="btn btn-danger" onclick="stopScanner()"><i class="fas fa-stop"></i> Hentikan</button></div></div><div class="info-card"><i class="fas fa-info-circle"></i> <strong>Cara Penggunaan:</strong><ol><li>Klik "Mulai Scan"</li><li>Izinkan akses kamera</li><li>Arahkan ke QR Code anggota</li><li>Nama anggota akan muncul dengan JELAS dan BESAR</li><li>Absensi otomatis tercatat dengan suara BEEP</li><li>Klik Flash untuk menyalakan lampu kamera</li><li>Jika salah scan, bisa langsung EDIT atau HAPUS</li></ol></div></section><section id="generatorSection" class="content-section"><div class="card"><div class="card-header"><i class="fas fa-print"></i><h3>Cetak QR Code Anggota</h3><small>QR Code berisi data diri lengkap</small></div><div class="qr-generator-grid"><div class="qr-card-preview"><div class="qr-display" id="qrCodeDisplay"><i class="fas fa-qrcode" style="font-size: 80px; color: #94a3b8;"></i><p style="margin-top:10px;">Pilih anggota dan klik Generate</p></div><div class="download-buttons"><button class="btn btn-primary" onclick="downloadQRAsPNG()"><i class="fas fa-image"></i> Download PNG</button><button class="btn btn-success" onclick="downloadQRAsPDF()"><i class="fas fa-file-pdf"></i> Download PDF</button></div></div><div><select id="memberSelect" class="input-group" style="width:100%; padding:12px;"><option value="">-- Pilih Anggota Aktif --</option></select><button class="btn btn-primary" style="width:100%; margin-top:16px;" onclick="generateMemberQR()"><i class="fas fa-qrcode"></i> Generate QR Code</button><div id="memberInfoDisplay" class="member-info-card" style="margin-top:16px;"><h4><i class="fas fa-info-circle"></i> Informasi</h4><p style="text-align:center;">Pilih anggota dan klik "Generate QR Code"</p></div></div></div></div></section><section id="membersSection" class="content-section"><div class="card"><div class="card-header"><i class="fas fa-users"></i><h3>Data Anggota IRMANUFA</h3><small>Total: ${AppState.members.length} anggota (${activeCount} Aktif, ${passiveCount} Pasif)</small><div style="margin-left:auto; display: flex; gap: 8px;"><button class="btn btn-secondary" onclick="resetMembersToDefault()" style="padding: 8px 16px;"><i class="fas fa-undo"></i> Reset</button><button class="btn btn-primary" onclick="showAddMemberModal()" style="padding: 8px 16px;"><i class="fas fa-plus"></i> Tambah</button></div></div><div class="search-box"><input type="text" id="memberSearch" placeholder="Cari nama, kode, atau divisi..." onkeyup="renderMemberTable(this.value)"></div><div class="table-container"><table class="data-table"><thead><tr><th>Kode</th><th>Nama</th><th>Divisi</th><th>Jabatan</th><th>Status</th><th>Hari Ini</th><th>Aksi</th></tr></thead><tbody id="memberTableBody"></tbody></table></div></div></section><section id="reportsSection" class="content-section"><div class="card"><div class="card-header"><i class="fas fa-chart-line"></i><h3>Laporan Absensi</h3></div><div class="kegiatan-input"><label><i class="fas fa-tag"></i> Nama Kegiatan:</label><input type="text" id="kegiatanName" placeholder="Contoh: Rapat Koordinasi, Kajian Rutin, dll"></div><div class="report-header"><div class="report-date"><label>Dari:</label><input type="date" id="startDate"><label>Sampai:</label><input type="date" id="endDate"></div></div><div class="action-buttons"><button class="btn btn-primary" onclick="generateReport()"><i class="fas fa-file-pdf"></i> Export PDF</button><button class="btn btn-success" onclick="exportToExcel()"><i class="fas fa-file-excel"></i> Export Excel</button><button class="btn btn-info" onclick="showWhatsAppModal()"><i class="fab fa-whatsapp"></i> Kirim via WA</button><button class="btn btn-danger" onclick="resetAllData()"><i class="fas fa-trash"></i> Reset Absensi</button></div></div><div class="stats-grid"><div class="stat-card"><div class="stat-icon"><i class="fas fa-database"></i></div><div class="stat-number">${AppState.attendance.length}</div><div class="stat-label">Total Data</div></div><div class="stat-card"><div class="stat-icon"><i class="fas fa-qrcode"></i></div><div class="stat-number">${AppState.attendance.filter((a) => a.method === "qr").length}</div><div class="stat-label">QR Code</div></div><div class="stat-card"><div class="stat-icon"><i class="fas fa-user-check"></i></div><div class="stat-number">${new Set(AppState.attendance.map((a) => a.memberId)).size}</div><div class="stat-label">Pernah Absen</div></div></div></section></div></div>`;
  updateAllDisplays();
  updateMemberSelect();
  const kegiatanInput = document.getElementById("kegiatanName");
  if (kegiatanInput) {
